...
from KRISHNA.database.pmpermitdb import get_approved_users, pm_guard
import KRISHNA.database.pmpermitdb as KRISHNA
...
async def pmguard(client, message):
    ...
    if not arg:
        ...
        return
    await KRISHNA.set_limit(int(arg))
...
async def setpmmsg(client, message):
    ...
    if not arg:
        ...
        return
    if arg == "default":
        await KRISHNA.set_block_message(KRISHNA.BLOCKED)
        await message.edit("**Block message set to default**.")
        return
    await KRISHNA.set_block_message(f"`{arg}`")
...
async def allow(client, message):
    ...
    pmpermit, pm_message, limit, block_message = await KRISHNA.get_pm_settings()
    await KRISHNA.allow_user(chat_id)
...
async def deny(client, message):
    chat_id = message.chat.id
    await KRISHNA.deny_user(chat_id)
