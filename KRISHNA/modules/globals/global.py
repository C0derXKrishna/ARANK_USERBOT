...
DEVS = [5561431531, 6895470102]
from KRISHNA.helper.PyroHelpers import get_ub_chats
from KRISHNA.modules.basic.profile import extract_user, extract_user_and_reason
from KRISHNA.database import gbandb as KRISHNA
from KRISHNA.database import gmutedb as Gmute
from KRISHNA.modules.help import add_command_help
...
async def gban_user(client: Client, message: Message):
    ...
    if (await KRISHNA.gban_info(user.id)):
    ...
    for gokid in f_chats:
        try:
            ...
            er += 1
    await KRISHNA.gban_user(user.id)
    ok.append(user.id)
    msg = (
        r"**\\#GBanned_User//**"
...
async def ungban_user(client: Client, message: Message):
    ...
    try:
        if not (await KRISHNA.gban_info(user.id)):
        ...
        for good_boi in ung_chats:
            try:
                ...
                er += 1
        await KRISHNA.ungban_user(user.id)
        msg = (
            r"**\\#UnGbanned_User//**"
            f"\n\n**First Name:** [{user.first_name}](tg://user?id={user.id})"
            f"\n**User ID:** `{user.id}`"
...
if ok:
 ...
 async def globals_check(client: Client, message: Message):
    ...
    if not user_id:
        return
    if (await KRISHNA.gban_info(user_id)):
        try:
