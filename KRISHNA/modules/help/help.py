...
from KRISHNA import app, CMD_HELP
from KRISHNA.helper.PyroHelpers import ReplyCheck
from KRISHNA.helper.utility import split_list
...
async def module_help(client: Client, message: Message):
    ...
    if len(cmd) > 1:
        ...
        await message.edit("ᴋʀɪꜱʜɴᴀ ᴘᴀᴘᴀ ᴄᴏᴍɪɴɢ..😈")
        try:
...
async def module_helper(client: Client, message: Message):
    ...
    if len(cmd) > 1:
        ...
        ac = PrettyTable()
        ac.header = False
        ac.title = "KRISHNA-UserBot Plugins"
        ac.align = "l"
        for x in split_list(sorted(CMD_HELP.keys()), 2):
            ac.add_row([x[0], x[1] if len(x) >= 2 else None])
        await edit_or_reply(
